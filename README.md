...
Fork of [bobzhang/toml](https://github.com/moonbit-community/toml-parser) (v0.4.1) with the following fix:

- **`create_array_of_tables` path-slice bug**: `path[:-1]` aborts with "View index out of bounds" on single-element paths (e.g. `[[models]]`). Changed back to `path[:path.length() - 1]`. This caused `hnlyxiaobing/MBOpenClacky`'s `[[models]]` config test to panic.
